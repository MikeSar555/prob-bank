...
public class ProbBankApplication {
	...
	public String sayHello(@RequestParam(value = "name", defaultValue = "World") String name) {
		Flux<GatePay> payFlux = prob.findByParamsContaining("902717");
		ArrayList<GatePay> g_list = (ArrayList<GatePay>) payFlux.collectList().share().block();
		int g_list_size = g_list.size();
		for(GatePay g:g_list) {
			System.out.println("Id "+g.getId()+" date "+g.getEventDate()+" amount "+g.getAmount()+" params  "+g.getParams()+" fio  "+g.getClientFio() );
